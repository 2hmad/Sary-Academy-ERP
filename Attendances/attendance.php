                ...
                <form method="POST" style="margin-top: 2%;margin-bottom:5%" enctype="multipart/form-data">
                    <div style="display:block;text-align:center">
                        <select name="role" required>
                            <?php
                            if(!isset($_GET['type'])) {
                                echo '<option value="" hidden>Choose Type</option>';
                            } else {
                                $type_get = $_GET['type'];
...
if(isset($_GET['month']) && $totalItems > 0) {
    ...
    <form method="POST" style="width:auto" action="export_attendance.php?month='.$month_get.'">
        <button class="btn btn-success" name="export" type="submit" style="width: auto;margin-bottom:2%;text-transform:capitalize;float:left"><i class="fas fa-file-csv"></i> Export '.$month_get.' Attendences</button>
    </form>
    ';
}
?>
