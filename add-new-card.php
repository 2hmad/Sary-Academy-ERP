        ...
        <div class="col-lg">
            ...
            <div class="row" style="background:white;padding:20px;box-shadow:0 0 15px -9px rgba(0, 0, 0, 0.25);border-radius:5px;margin-top:3%">
                <form method="POST" style="margin-top: 2%;" enctype="multipart/form-data">
                    ...
                    <?php
                        $today = date('dHi');
                        $startDate = date('dH', strtotime('09:06'));
                        $range = $today - $startDate;
                        $rand = rand(0, $range);                        
                    ?>
                    <input type="text" name="code" value="<?php echo "SA-$rand" . ($startDate + $rand); ?>" disabled>
...
if(isset($_POST['create-card'])) {
    ...
    if($_FILES["profile-pic"]["name"] !== "") {
        ...
        if(move_uploaded_file($_FILES["profile-pic"]["tmp_name"], $targetFilePath)){
            $sql = "INSERT INTO cards (name, phone, birthday, gender, code, profile_pic, hours) VALUES ('$name', '$phone', '$birthday', '$gender', '$code', '$fileName', $hours)";
            $query = mysqli_query($connect, $sql);
        }
    } else {
        $sql = "INSERT INTO cards (name, phone, birthday, gender, code, hours) VALUES ('$name', '$phone', '$birthday', '$gender', '$code', $hours)";
        $query = mysqli_query($connect, $sql);
    }
}
?>
